...
The Node.js route uses `takumi-js/response`, with `takumi-js` and `@takumi-rs/core` externalized as required by [Takumi's Next.js integration](https://takumi.kane.tw/docs/integration/nextjs). Built-in Geist avoids remote font requests. Rendering never calls Notion or accepts arbitrary image/title query parameters. Missing or unpublished articles return 404; existing aliases resolve the canonical article.
...
Social images use Vercel's public production domain (`VERCEL_PROJECT_PRODUCTION_URL`) in production. Preview builds prefer `VERCEL_BRANCH_URL`, falling back to `VERCEL_URL` if there is no branch alias. This follows Next.js's social-image URL selection and avoids using a protected, unique deployment URL for public production cards. All Vercel hostnames are prefixed with HTTPS. Outside Vercel, the URL falls back to `site.origin`. Canonical article URLs, sitemap URLs, and site identity continue to use the final production origin. See [Vercel's system variables](https://vercel.com/docs/environment-variables/system-environment-variables).

Keep `@takumi-rs/core` installed directly and both Takumi packages externalized. Bundling the wrapper can embed its native loader's build-machine path and omit the platform binary from the deployed function. `pnpm build` verifies that Next.js's social-image function trace includes an existing native binding, so this failure is caught before deployment.
