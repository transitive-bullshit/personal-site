...
Social image URLs use `https://${VERCEL_URL}` on Vercel so preview and production deployments render their own version of the card. `VERCEL_URL` is the built-in deployment hostname, available during build and runtime ([Vercel documentation](https://vercel.com/docs/environment-variables/system-environment-variables#vercel_url)). Outside Vercel, the URL falls back to `site.origin`. Canonical article URLs, sitemap URLs, and site identity continue to use the production origin.
