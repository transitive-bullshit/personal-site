...
import { loadEnv, lockSync, publishSnapshot, readSnapshot } from './io'
...
export async function main() {
  ...
  if (values.help) {
    console.log(
      'pnpm content:sync [--dry-run] [--force] [--prune] [--accept-slug-changes]\nOnly --prune removes missing pages; only --accept-slug-changes renames existing paths.'
    )
    return
  }
  await loadEnv()
  ...
  const release = values['dry-run'] ? undefined : await lockSync()
  try {
    ...
    // A HEAD against our namespace verifies read access without mutating the bucket.
    ...
    const normalizer = new Normalizer(api, routes, (source, url, refresh) =>
      importer.import(source, url, refresh)
    )
    ...
    normalizer.warnings.forEach((warning) => console.warn(warning))
    ...
    const addedPlaceholders = await backfillPlaceholders(snapshot, {
      ...options,
      cache
    })
